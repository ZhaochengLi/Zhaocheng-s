...
Smarket.1=Smarket[train,]
dim(Smarket.2005)
Direction.2005=Direction[!train]
Smarket.2005$Direction=NULL
Smarket.2005
Direction.2005
# Logistic Regression
...
library(rpart)
# grow tree 
rfit=rpart(Direction~Lag1+Lag2,method="class", data=Smarket.1)
rfit
rpred=predict(rfit,Smarket.2005,type="class")
rpred

table(Smarket$Direction[c(999:1250)],rpred)

summary(rpred)
summary(rpred1)

printcp(rfit) # display the results 
plotcp(rfit) # visualize cross-validation results 
summary(rfit) # detailed summary of splits
str(Smarket.1)
lm.fit = lm(Direction~Lag1+Lag2, data=Smarket.1)
lm.pred=predict(lm.fit,Smarket.2005)
lm.pred
